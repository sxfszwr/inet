...
bool InstantServer::canProcessPacket()
{
    ...
    if (provider->canPopSomePacket(inputGatePathStartGate) && consumer->canPushSomePacket(outputGatePathEndGate))
        return true;
    else {
        auto packet = provider->canPopPacket(inputGatePathStartGate);
...
void InstantServer::handleCanPopPacket(cGate *gate)
{
