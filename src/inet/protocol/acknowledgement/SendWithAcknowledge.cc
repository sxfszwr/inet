...
void SendWithAcknowledge::handleMessage(cMessage *message)
{
    ...
    producer->handlePushPacketConfirmation(packet, inputGate->getPathStartGate(), false);
    delete message;
}
...
void SendWithAcknowledge::processPacket(Packet *packet)
{
    ...
    if (protocol == &IProtocol::acknowledge) {
        ...
        producer->handlePushPacketConfirmation(packet, inputGate->getPathStartGate(), true);
